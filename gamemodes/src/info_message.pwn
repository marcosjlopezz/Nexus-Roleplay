...
public SendInfoMessage(playerid, const reason[])
{
    for(new cycle; cycle < 3; cycle++)
    {
        if(!TextDrawsInfoMessage[playerid][cycle][Use])
        {
            ...
            strcat(TextDrawsInfoMessage[playerid][cycle][Text], reason, 800);
            ...
            SetTimerEx("TimerHideInfoMessage", 5000, false, "i", playerid);
        ...
        if(!TextDrawsInfoMessage[playerid][cycle][Use])
        {
            if(cycle != 3 - 1)
            {
                if(TextDrawsInfoMessage[playerid][cycle + 1][Use])
                {
                    ...
                    strcat(TextDrawsInfoMessage[playerid][cycle][Text], TextDrawsInfoMessage[playerid][cycle + 1][Text], 800);
        ...
        else if(TextDrawsInfoMessage[playerid][cycle][Use])
        {
            if(cycle != 0)
            {
                if(!TextDrawsInfoMessage[playerid][cycle - 1][Use])
                {
                    ...
                    strcat(TextDrawsInfoMessage[playerid][cycle - 1][Text], TextDrawsInfoMessage[playerid][cycle][Text], 800);
...
stock MinPosYInfoMessage(playerid, TDN)
{
    if(TDN == 0)
    {
        TextDrawsInfoMessage[playerid][TDN][MinPosY] = PI[playerid][pWANTED_LEVEL] == 0 ? 130.000000 : 153.0;
    }
    else
    {
...
stock CreateTDN(playerid, TDN)
{
    if(TextDrawsInfoMessage[playerid][TDN][Use] == 1)
    {
        ...
        PlayerTextDrawBackgroundColor(playerid, TextDrawsInfoMessage[playerid][TDN][TextDraw], 0x000000FF);
...
SendInfoMessagef(playerid, const text[], {Float, _}:...)
{
	static
	    args,
	    str[192];
