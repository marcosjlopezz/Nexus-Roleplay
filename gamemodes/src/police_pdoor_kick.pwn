...
CMD:allanar(playerid, params[]) 
{
	...
	if(PLAYER_TEMP[playerid][pt_WORKING_IN] != WORK_POLICE) return SendClientMessagef(playerid, -1, "No estás de servicio como policía.");
	...
	if(PLAYER_TEMP[playerid][pt_LAST_PICKUP_ID] == 0) return SendClientMessagef(playerid, -1, "No estás en el lugar adecuado.");
