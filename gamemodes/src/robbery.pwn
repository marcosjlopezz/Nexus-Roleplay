...
hook OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ) {
    if(PI[playerid][pCREW]) {
        new now = gettime(), crewIndex = PLAYER_TEMP[playerid][pt_CREW_INDEX];
        ...
        if(enterExitIndex != -1 && ENTER_EXIT[enterExitIndex][ee_ROBBABLE]) {
            if(ENTER_EXIT[enterExitIndex][ee_TIMER] == -1) {
                if(now > CREW_INFO[crewIndex][crew_LAST_ROBBERY] + 300) { // 5 min
...
hook OnPlayerEnterInterior(playerid, index) {
    if(ENTER_EXIT[index][ee_TIMER] != -1) {
        ...
        if(PI[playerid][pCREW] && ENTER_EXIT[index][ee_LAST_CREW_INDEX_ROBBERY] == PLAYER_TEMP[playerid][pt_CREW_INDEX]) {
	...
	for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
	{
		if(IsPlayerConnected(i))
		{
			if(PI[i][pCREW])
			{
				...
				{
					if(PI[i][pSTATE] == ROLEPLAY_STATE_INTERIOR && PLAYER_TEMP[i][pt_GAME_STATE] == GAME_STATE_NORMAL)
					{
...
StartShopAttack(crew_index, enter_exit_index)
{
    ...
    for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
	{
		if(IsPlayerConnected(i))
		{
			if(PI[i][pCREW] && PLAYER_TEMP[i][pt_CREW_INDEX] == crew_index && PLAYER_TEMP[i][pt_INTERIOR_INDEX] == enter_exit_index)
			{
...
public UpdateShopAttack(crew_index, enter_exit_index) {
    ...
    if(ENTER_EXIT[enter_exit_index][ee_ROBBERY_PROGRESS] >= 120.0 && ENTER_EXIT[enter_exit_index][ee_STEP] == 2) {
    ...
    else if(ENTER_EXIT[enter_exit_index][ee_ROBBERY_PROGRESS] >= 90.0 && ENTER_EXIT[enter_exit_index][ee_STEP] == 1) {
        new Float:x, Float:y, Float:z;
        ...
        format(message_police, sizeof message_police, "{"#PRIMARY_COLOR"}[Central policía] {FFFFFF}La banda '%s' ha atracado una tienda en {"#PRIMARY_COLOR"}%s, %s.", CREW_INFO[crew_index][crew_NAME], city, zone);
