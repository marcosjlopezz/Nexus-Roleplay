...
public CheckAFKPos(playerid) 
{
    ...
    if(ac_Info[CHEAT_AFKPOS][ac_Enabled]) 
    {
        if(PI[playerid][pSTATE] != ROLEPLAY_STATE_JAIL && GetPlayerDistanceFromPoint(playerid, PAFKLastPositions[playerid][0], PAFKLastPositions[playerid][1], PAFKLastPositions[playerid][2]) <= AFK_POS_DISTANCE) {
            new now = gettime();
